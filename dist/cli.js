    ...
    try {
      if (branch) {
        ...
        for (const tag of tags) {
          const matchesFormat = stableOnly ? /^v\d+\.\d+\.\d+$/.test(tag.name) : /^v\d+\.\d+\.\d+/.test(tag.name);
...
function getLatestTag(branch, stableOnly = false) {
  try {
...
function getLocalCommits(base, head) {
  try {
    let range = head;
    if (base) {
      range = `${base}..${head}`;
...
function parseCommitType(message) {
  ...
  const hasBreakingChange = firstLine.includes("!:") || message.includes("BREAKING CHANGE:") || message.includes("BREAKING-CHANGE:");
...
function analyzeCommits(commits) {
  let releaseType = "none" /* NONE */;
  for (const commit of commits) {
    const commitType = parseCommitType(commit.message);
    if (commitType === "major" /* MAJOR */) {
      return "major" /* MAJOR */;
    }
    if (commitType === "minor" /* MINOR */) {
      releaseType = "minor" /* MINOR */;
    }
    if (commitType === "patch" /* PATCH */ && releaseType === "none" /* NONE */) {
      releaseType = "patch" /* PATCH */;
    }
  }
  return releaseType;
}
function calculateNextVersion(currentVersion, releaseType, isMainBranch, suffix) {
  if (releaseType === "none" /* NONE */) {
    return null;
  }
  ...
  if (isMainBranch) {
    switch (releaseType) {
      case "major" /* MAJOR */:
        nextVersion = import_semver.default.inc(baseVersion, "major");
        break;
      case "minor" /* MINOR */:
        nextVersion = import_semver.default.inc(baseVersion, "minor");
        break;
      case "patch" /* PATCH */:
        nextVersion = import_semver.default.inc(baseVersion, "patch");
        break;
    }
  } else {
    ...
    switch (releaseType) {
      case "major" /* MAJOR */:
        nextStableVersion = import_semver.default.inc(baseVersion, "major");
        break;
      case "minor" /* MINOR */:
        nextStableVersion = import_semver.default.inc(baseVersion, "minor");
        break;
      case "patch" /* PATCH */:
        nextStableVersion = import_semver.default.inc(baseVersion, "patch");
        break;
    }
...
async function getCurrentVersion(options = {}) {
  ...
  let latestTag = getLatestTag(currentBranch, isMainBranch);
...
async function getNextVersion(options = {}) {
  ...
  let tagSource = "none";
  if (latestTag) {
    baseCommitSha = getTagCommitSha(latestTag);
    if (baseCommitSha) {
      tagSource = "local git";
    } else {
...
program2.name("ghsemver").description("Semantic versioning tool based on GitHub commit history").version("1.0.0");
program2.command("current").description("Get current version").option("-b, --branch <branch>", "Specify branch").option("-m, --main-branch <mainBranch>", "Specify main branch").option("--log", "Enable verbose logging").action(async (options) => {
  try {
...
program2.command("next").description("Calculate next version").option("-b, --branch <branch>", "Specify branch").option("-m, --main-branch <mainBranch>", "Specify main branch").option("-s, --suffix <suffix>", "Prerelease suffix (non-main branch)").option("--log", "Enable verbose logging").action(async (options) => {
  try {
